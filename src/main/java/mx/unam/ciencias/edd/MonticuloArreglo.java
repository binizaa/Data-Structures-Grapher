...
public class MonticuloArreglo<T extends ComparableIndexable<T>>
    ...
    public MonticuloArreglo(Iterable<T> iterable, int n) {
        ...
        for(T elemento : iterable){
            elemento.setIndice(cnt);
            this.arreglo[cnt++] = elemento;
        }
        this.elementos = n;
    }
    ...
    @Override public T elimina() {
        if(elementos == 0) throw new IllegalStateException("Monticulo vacio");

        T menor = null;

        for(T elemento : arreglo){
            if(elemento != null 
                && (menor == null || elemento.compareTo(menor) < 0)){
                    menor = elemento;
            }
        }
        
        int index = menor.getIndice();
        arreglo[index] = null;
        menor.setIndice(-1);
        elementos--;
    ...
    @Override public T get(int i) {
        if(!validIndex(i)) throw new NoSuchElementException("Index invalido");
        return arreglo[i];
    }
